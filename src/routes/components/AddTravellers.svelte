<script>
    ...
    $: passengers = [{id: 0, name: '', phone: '', email: '', theme: 'stone'}];
    $: saved = [];
    let tally = 0;

    function addTraveler() {
        passengers = [{id: tally + 1, name: '', phone: '', email: '', theme: 'stone'}, ...passengers]
        tally++;
    }
    ...
    function select(id, passenger) {
        ...
        passengers[passengers.indexOf(selected)] = {id: passengers[passengers.indexOf(selected)].id,...passenger};
    }
...
<div class="card">
    ...
    <div class:grid={passengers.length > 1} class="travelers-container">
                    ...
                    <details class="my-2" >
                        ...
                        <div class="saved">
                            ...
                            {#each saved as s}
                                ...
                                <a class="saved-entry" on:click={() => {select(p.id, s)}}>{s.name}</a>
                            {/each}
                        </div>
                    </details>
